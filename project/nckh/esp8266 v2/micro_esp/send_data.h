...
void Serial_make::send_byte(int x)
{
  ...
  Serial.println("[SEND] Send byte ok" );
}
...
void  Serial_make::send(String s)
{
  Serial.println("[SEND] Send string: " + s);
  for(int i= 0; i<s.length(); i++)
    {
      send_byte((int)s[i]);
    }
    Serial.println("[SEND] Send string ok : " + s);
}
...
void Serial_make::receive_byte()
{
  ...
  Serial.print("[BIT] nhan");
  Serial.println(Bit[sl_bit-1]);
  if (sl_bit == 8){
    sl_bit = 0;
    int ch = 0;
...
void Serial_make::start_key()
{
    ...
    {
      sl_bit = 0;
      Serial.println("[BIT] Key start");
    }else {
      Serial.println(sl_bit);
    }
}
