// server ----------------------------------------------------
/*
 * program setup deface user
 * use coockie save info user same session 
 * each user have a coocke randome from anchars
 *  default:
 *    connect to wifi ESP8266  Wifi broadcast 
 *    browser to 192.168.4.1
 *    /login: login and setup to systen
 *    /test: test sound\
 *    /status: staus in wifi
 *    
 *  void make_session() // make a new key in session 
 *   32 char random from abcdefghijklmnopqrstuvwxyz0123456789
 *  
 *  bool  check_user_login()
 *  if session same user login then login accect
 *  elaw login agne
 *  return 1 is accect
 *  else return 9
 *  
 *  
 *  void func_header() html in header and forword
 *  
 *  void func_login() form login 
 *    default:
 *      admin
 *      admin
 *      
 * viod func_send_data() send data for test send data to arduino pro micro
 *  set data in url is data send to arduino
 *  
 *  
 * void func_write_file() save new data for system in SPIFFS file 
 * 
 * func_not_found() web not found\
 * 
 * 
 * 
 */
//
String anchars = "abcdefghijklmnopqrstuvwxyz0123456789" ;// truong key ..... library data key
String sessioncookie = "";  // sessionkie duy tri dang nhap 
void make_session() { // make session
  sessioncookie = "";
...
void setup_server() // main setup 
{
  server.on("/", func_header);// open server and link share to user
  ...
  // set cookie and connect to server
